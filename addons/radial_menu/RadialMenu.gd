...
func place_buttons():
	...
	for button in buttons:
		...
		var x = center.x + cos(angle) * (half_size + size.x - icon_shift)
		var y = center.y + sin(angle) * (half_size + size.y - icon_shift)
	...
	self.do_modulate()
	$RadialMenu/CursorPos.refresh_index()
