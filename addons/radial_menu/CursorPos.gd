...
func get_cursor_index():
	...
	var angle = PI - atan2(normalized_cursor.y, normalized_cursor.x) + (PI/2)
	...
	angle -= index_offset/2
	angle = (angle if angle >= 0 else angle + (PI * 2)) if angle <= (PI*2) else angle - (PI * 2)
	...
	to_return = int((floor(to_return) + 1)) % self.count
